...
includedir = abspath("/usr/local/Cellar/poppler/0.87.0/include/poppler/glib")
headerfiles = joinpath.(includedir, readdir(includedir))
...
wc = init(; headers = headerfiles,
            output_file = joinpath(@__DIR__, "..", "src", "poppler_glib.jl"),
            common_file = joinpath(@__DIR__, "..", "src", "poppler_glib_common.jl"),
            clang_includes = [includedir, CLANG_INCLUDE],
            clang_args = ["-I", includedir],
            header_wrapped = (root, current) -> root == current,
            header_library = x -> "poppler",
